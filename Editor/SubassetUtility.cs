...
namespace m4k
{
    ...
    internal static class SubassetUtility
    {
            ...
            IEnumerable<Object> candidateSubassets)
        {
            ...
            if(unreferencedSubassets.Count<Object>() < 1) return;
            string s = "";
            foreach(var i in unreferencedSubassets) {
                s += $"{i.name}\n";
            }
            if(!EditorUtility.DisplayDialog("Delete unreferenced subassets", $"Do you want to delete[{unreferencedSubassets.Count<Object>()}]:\n{s}", "Confirm", "Cancel"))
                return;
